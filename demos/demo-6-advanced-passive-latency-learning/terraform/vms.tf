...
resource "azurerm_virtual_machine_extension" "backend_win_setup" {
  ...
  settings = jsonencode({
    commandToExecute = join(" && ", [
      ...
      "powershell -ExecutionPolicy Bypass -Command \"choco install git golang -y --no-progress\"",
      "powershell -ExecutionPolicy Bypass -Command \"$env:Path = [Environment]::GetEnvironmentVariable('Path','Machine'); cd C:\\opengslb; & 'C:\\Program Files\\Git\\bin\\git.exe' clone --depth 1 --branch ${var.opengslb_git_branch} ${var.opengslb_git_repo} src\"",
      "powershell -ExecutionPolicy Bypass -Command \"$env:Path = [Environment]::GetEnvironmentVariable('Path','Machine'); cd C:\\opengslb\\src; & go build -o C:\\opengslb\\opengslb.exe ./cmd/opengslb\"",
